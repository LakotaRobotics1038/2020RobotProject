...
public class Endgame implements Subsystem {
    ...
    private final int MAX_COUNT = 0; // was 220, that is the value at the bottom of endgame, top of endgame should be
                                     // 0 Needs to be updated with actual value
    public final int MIN_COUNT = -220; // 201 is where it hits the safety

    public enum directionsOptions {
        preExtend, extending, retracting, stop
    };
    ...
    public CANSpark1038 motor = new CANSpark1038(SPARK_PORT, MotorType.kBrushless); // negative value makes the motor
    ...
    public void periodic() {
        ...
        System.out.println("Endgame state" + endgameState + " " + "Is motor locked" + MotorIsLocked + " "
                + "Encoder count" + encoder.getPosition() + " " + "Is safety locked" + safetyIsLocked);
        switch (Directions) {
            default:
            ...
            case preExtend:
                ...
                if (encoder.getPosition() > (MIN_COUNT - 10) && !preExtendState) { // 210 is close to the bottom, but
                                                                                   // not bottom'd out DO NOT BOTTOM OUT
                                                                                   // THE ROBOT
                    safetyUnlock();
                    motorUnLock();
                    motor.set(-.25); // moves the motor counter-clockwise to release the tension on the rachet and
                                     // gear
            ...
            // sets the motor to the extending state. This will have Endgame go up until it
            // reaches the max count.
            case extending:
                // Extends Endgame
                // motorUnLock();
            ...
            // sets the motor to the retracting state. This will have Endgame go down until
            // it reaches the minium count.
            case retracting:
                motorLock();
                // Retracts Endgame
            ...
            // sets the motor to the stop state. This will set the motor to stop and lock
            // the motor. Call this after every single case so Endgame doesn't move back up
            case stop:
                // Stops Endgame
                // motor.set(-.8);
                // Thread.sleep(10);
    ...
    public void onJoyStick(double joystickValue) {
        ...
        // else if (endgameState == true && (Directions == directionsOptions.stop)) {
        // //Checks to see if the button has been pressed before
